...
def getXFromMinImAtY(dataset: Dataset, point):
    return dataset.getHorizontalAt(point).index(min(dataset.getHorizontalAt(point)))

def getXFromMaxImAtY(dataset: Dataset, point):
    return dataset.getHorizontalAt(point).index(max(dataset.getHorizontalAt(point)))


def getYFromMinImAtX(dataset: Dataset, point):
    return dataset.getVerticalAt(point).index(min(dataset.getVerticalAt(point)))

def getYFromMaxImAtX(dataset: Dataset, point):
    return dataset.getVerticalAt(point).index(max(dataset.getVerticalAt(point)))
